...
impl RouteManager {
    ...
    pub async fn switch_target(
        ...
        if let Some(existing) = config.find_route(host_port) {
            let route = config.routes.iter_mut().find(|r| r.host_port == host_port).unwrap();
    ...
    pub async fn stop_port(&self, host_port: u16) -> Result<()> {
        self.proxy_manager.stop_port(host_port).await
    }
}
